...
class MenuEntradaViewModel @Inject constructor() : ViewModel() {
    ...
    fun nombreCompleto(lista:List<String>) {
        _nombreCompletoDB.value = lista
...
fun telefonoDB(lista:List<String>) {
    ...
    fun sumarInidice(numeroSumar:Int) {

        val sumado = numeroSumar + 1
    ...
    fun imprimirInformacion(datoQueRecorrer:String):List<String> {
        val db = FirebaseFirestore.getInstance()
        ...
        db.collection("usersInformation").get()
            .addOnSuccessListener { result ->
                for (document in result) {
                    val nombre = document.getString(datoQueRecorrer)
